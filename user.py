...
def update_pending_user(user_id):
    ...
    temp_user_data = (data['Username'], data['Email'], data['FirstName'],
                      data['LastName'], data['Phone'], data['RoleID'], data['FacultyName'], user_id)
...
def get_all_users():
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute(
        "SELECT UserID , PositionEnglish , LastName , FirstName  ,Username , FacultyName, RoleID FROM Users")
...
def get_all_users_except_students():
    conn = get_db()
    cursor = conn.cursor(dictionary=True)
    # Assuming student role ID is 5
    cursor.execute(
        "SELECT Userid, Username, FirstName , LastName , SignatureLocation , SealLocation , ProfilePicLocation FROM Users WHERE RoleID != 5")
    users = cursor.fetchall()
    cursor.close()
    conn.close()
    return jsonify({'users': users, "statuscode": 200}), 200
...
def get_only_student_users():
    ...
    # Assuming student role ID is 5
    cursor.execute(
        "SELECT Userid, Username, FirstName , LastName , SignatureLocation , SealLocation FROM Users WHERE RoleID = 5")
    users = cursor.fetchall()
    cursor.close()
    conn.close()
    return jsonify({'users': users, "statuscode": 200}), 200
...
def send_email(email, username, roleid):
    ...
    email_body = render_template(
        'email_template_approved_user_notification.html', username=username, rolename=rolename)
...
def user_management_overview():
    ...
    cursor.execute(
        "SELECT COUNT(*) AS total_admin FROM Users WHERE RoleID = 1")
    total_admin = cursor.fetchone()
    print(total_admin['total_admin'])

    cursor.execute(
        "SELECT COUNT(*) AS total_teacher FROM Users WHERE RoleID = 4")
    total_teacher = cursor.fetchone()
    print(total_teacher['total_teacher'])

    cursor.execute(
        "SELECT COUNT(*) AS total_researcher FROM Users WHERE RoleID = 2")
    total_researcher = cursor.fetchone()
    print(total_researcher['total_researcher'])

    cursor.execute(
        "SELECT COUNT(*) AS total_reviewer FROM Users WHERE RoleID = 3")
    total_reviewer = cursor.fetchone()
    print(total_reviewer['total_reviewer'])

    cursor.execute(
        "SELECT COUNT(*) AS total_student FROM Users WHERE RoleID = 5")
...
def get_specific_user_minimum(user_id):
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute(
        "SELECT Username , FirstName , LastName , ProfilePicLocation FROM Users WHERE Userid = %s", (user_id,))
    user = cursor.fetchone()
    cursor.close()
    conn.close()
    print(jsonify(user))
...
def update_user_admin_mode(user_id):
    ...
    user_data = (data['Username'], data['Email'], data['FirstName'], data['LastName'], data['FullNameBangla'], data['PositionEnglish'], data['PositionBangla'], data['PositionHeldSince'], data['Gender'], data['Dateofbirth'], data['Phone'], data['Nid'], data['NidLocation'], data['FacultyName'], data['DepartmentName'], data['InstituteName'], data['InstituteLocation'], data['InstituteEmail'], data['PresentAddress'], data['PermanentAddress'], data['ProfilePicLocation'], data['SignatureLocation'], data['SealLocation'], data['SalaryScale'],
                 data['BasicPay'], data['HighestAcademicQualification'], data['HighestAcademicQualificationUniversity'], data['HighestAcademicQualificationCountry'], data['HighestAcademicQualificationYear'], data['AreaOfExpertise'], data['ExperienceInResearch'], data['Teaching'], data['TotalNumberOfCompleteProjects'], data['TotalNumberOfCompletePublications'], data['OngoingProjects'], data['StudentID'], data['StudentRegNo'], data['FirstEnrollmentSemester'], data['UndergraduateCGPALevel'], data['ReferencesOfLatestPublications'], data['RoleID'], user_id)
    cursor.execute(update_query, user_data)
    conn.commit()
    cursor.close()
    conn.close()
