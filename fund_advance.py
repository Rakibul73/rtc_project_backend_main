...
def check_a_project_advance_fund_applied_or_not(project_id ):
    ...
    cursor.execute("SELECT ProjectAdvanceFundID FROM ProjectAdvanceFund WHERE RequestForAdvanceFundDone > 0 AND ProjectID = %s" , (project_id,))
...
def create_advance_fund_request_for_specific_project():
    ...
    review_data = (data['ProjectID'], data['TotalBudget'], data['PiSignatureDate'] , data['RequestedAmount'] , data['ChairmanOfDepartmentSignatureDate'] , data['RequestForAdvanceFundDone'] , data['AdvanceFundRecievedDone'] , 0)
...
def check_a_project_advance_fund_confirmation_send_or_not(project_id ):
    ...
    cursor.execute("SELECT ProjectAdvanceFundID FROM ProjectAdvanceFund WHERE AdvanceFundRecievedDone > 0 AND ProjectID = %s" , (project_id,))
...
def update_confirm_advance_fund_recieved(project_id):
    conn = get_db()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("UPDATE ProjectAdvanceFund SET AdvanceFundRecievedDone = 1 WHERE ProjectID = %s" , (project_id,))
    conn.commit()
    cursor.close()
    conn.close()
...
def get_admin_fund_dashboard():
    ...
    cursor.execute("SELECT ProjectID FROM ProjectAdvanceFund WHERE AdvanceFundSendDone > 0")
    funded_projects = cursor.fetchall()
    print(funded_projects)
    funded_projects = len(funded_projects)
    
    cursor.execute("SELECT ProjectID FROM ProjectAdvanceFund WHERE RequestForAdvanceFundDone > 0")
    fund_request_queue = cursor.fetchall()
    print(fund_request_queue)
    fund_request_queue = len(fund_request_queue) - funded_projects
    
    cursor.execute("SELECT ProjectID FROM ProjectAdvanceFund WHERE AdvanceFundRecievedDone > 0")
...
def get_admin_advance_fund_queue_list():
    ...
    cursor.execute("SELECT * FROM ProjectAdvanceFund WHERE RequestForAdvanceFundDone > 0 AND AdvanceFundSendDone = 0")
...
def check_a_project_advance_fund_send_or_not(project_id ):
    ...
    cursor.execute("SELECT ProjectAdvanceFundID FROM ProjectAdvanceFund WHERE AdvanceFundSendDone > 0 AND ProjectID = %s" , (project_id,))
...
def update_advance_fund_send(project_id):
    conn = get_db()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("UPDATE ProjectAdvanceFund SET AdvanceFundSendDone = 1 WHERE ProjectID = %s" , (project_id,))
    conn.commit()
    cursor.close()
    conn.close()
