...
def get_total_number_of_all_dashboard():
    ...
    cursor.execute(
        "SELECT COUNT(*) AS total_project_report FROM Projects WHERE ProjectSoftCopyLocation IS NOT NULL")
...
def get_self_project_dashboard():
    ...
    cursor.execute(
        "SELECT COUNT(*) AS completed_projects FROM Projects WHERE ProjectStatus = 'Completed' AND CreatorUserID = %s", (current_user_id,))
    completed_projects = cursor.fetchone()
    print(completed_projects['completed_projects'])

    cursor.execute(
        "SELECT COUNT(*) AS total_projects FROM ProjectListWithUserID WHERE UserID = %s", (current_user_id,))
    total_projects = cursor.fetchone()
    print(total_projects['total_projects'])

    cursor.execute(
        "SELECT COUNT(*) AS pending_projects FROM Projects WHERE ProjectStatus = 'Pending' AND CreatorUserID = %s", (current_user_id,))
    pending_projects = cursor.fetchone()
    print(pending_projects['pending_projects'])

    cursor.execute(
        "SELECT COUNT(*) AS approved_projects FROM Projects WHERE ProjectStatus = 'Approved' AND CreatorUserID = %s", (current_user_id,))
    approved_projects = cursor.fetchone()
    print(approved_projects['approved_projects'])

    cursor.execute(
        "SELECT COUNT(*) AS rejected_projects FROM Projects WHERE ProjectStatus = 'Rejected' AND CreatorUserID = %s", (current_user_id,))
    rejected_projects = cursor.fetchone()
    print(rejected_projects['rejected_projects'])

    cursor.execute(
        "SELECT COUNT(*) AS running_projects FROM Projects WHERE ProjectStatus = 'Running' AND CreatorUserID = %s", (current_user_id,))
