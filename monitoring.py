...
def update_project_gantt_for_report(projectMonitoringReportID):
    ...
    for activity_data in data:
        ...
        user_data = (activity_data['ProjectID'], activity_data['Activity'],
                     start_date, end_date, activity_data['ActivityStatus'])
        print(user_data)
...
def update_project_budget_for_report(projectMonitoringReportID):
    ...
    for budget_data in data:
        ...
        user_data = (budget_data['ProjectID'], budget_data['SerialNo'], budget_data['Item'],
                     budget_data['Quantity'], budget_data['UnitPrice'], budget_data['TotalCost'])
        print(user_data)
        cursor.execute(insert_query, user_data)
        conn.commit()
...
def get_single_project_monitoring_history(projectID):
    ...
    cursor.execute(
        "SELECT * FROM ProjectMonitoringReport WHERE ProjectID = %s ORDER BY ProjectMonitoringReportID DESC", (projectID,))
...
def get_specific_project_monitoring_report(monitoringReportID):
    ...
    cursor.execute(
        "SELECT * FROM ProjectMonitoringReport WHERE ProjectMonitoringReportID = %s", (monitoringReportID,))
...
def list_monitoring_feedback_project_and_pi_can_see():
    ...
    cursor.execute(
        "SELECT ProjectID FROM ProjectListWithUserID WHERE UserID = %s", (current_user_id,))
...
def monitoring_panel_overview():
    ...
    cursor.execute(
        "SELECT COUNT(DISTINCT ProjectMonitoringReportID) AS assigned_monitoring_committee FROM ProjectReportListWithMonitoringCommitteeID")
...
def get_committeeuserid_for_specific_monitoring_report(monitoringReportID):
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute(
        "SELECT MonitoringCommitteeUserID FROM ProjectReportListWithMonitoringCommitteeID WHERE ProjectMonitoringReportID = %s", (monitoringReportID,))
...
def set_reviewer_for_specific_project():
    ...
    review_data = (data['ProjectMonitoringReportID'],
                   data['MonitoringCommitteeUserID'])
    cursor.execute(insert_query, review_data)
    conn.commit()
    cursor.close()
    conn.close()
...
def check_a_monitoring_report_feedback_given_or_not(monitoringReportID, user_id):
    ...
    cursor.execute("SELECT * FROM ProjectMonitoringFeedback WHERE ProjectMonitoringReportID = %s AND MonitoringCommitteeUserID = %s",
                   (monitoringReportID, user_id))
...
def get_all_feedback_for_specific_monitoring_committee_and_specific_report():
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT * FROM ProjectMonitoringFeedback WHERE ProjectMonitoringReportID = %s AND MonitoringCommitteeUserID = %s",
                   (data['ProjectMonitoringReportID'], data['MonitoringCommitteeUserID']))
...
def create_feedback_specific_monitoring_report():
    ...
    review_data = (data['ProjectMonitoringReportID'], data['ProjectID'], data['MonitoringCommitteeUserID'],
                   data['Observation'], data['Suggestions'], data['Recommendations'], data['Endorsement'], 0)
    cursor.execute(insert_query, review_data)
...
def get_all_monitoring_report_committee_has_given_feedback():
    ...
    cursor.execute(
        "SELECT * FROM ProjectMonitoringFeedback GROUP BY ProjectMonitoringReportID HAVING COUNT(*) = 3")
...
def get_feedback_for_specific_monitoring_report(monitoringReportID):
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute(
        "SELECT * FROM ProjectMonitoringFeedback WHERE ProjectMonitoringReportID = %s", (monitoringReportID,))
    feedback = cursor.fetchall()
    cursor.close()
    conn.close()
    return jsonify({'feedback': feedback, 'statuscode': 200}), 200
...
def update_picanviewornot_in_project_monitoring_feedback(monitoringReportID):
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute(
        "UPDATE ProjectMonitoringFeedback SET PiCanViewOrNot = 1 WHERE ProjectMonitoringReportID = %s", (monitoringReportID,))
    conn.commit()
    cursor.close()
    conn.close()
