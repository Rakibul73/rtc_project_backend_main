...
def get_my_fund_dashboard():
    ...
    cursor.execute(
        "SELECT ProjectID FROM Projects WHERE ProjectStatus = 'Approved' AND CreatorUserID = %s", (current_user_id,))
    ...
    cursor.execute("SELECT ProjectID FROM ProjectAdvanceFund WHERE ProjectID IN (SELECT ProjectID FROM Projects WHERE ProjectStatus = 'Approved' AND CreatorUserID = %s) AND AdvanceFundSendDone > 0", (current_user_id,))
    advancefunded_project = cursor.fetchall()
    recieved_advance_fund = len(advancefunded_project)

    cursor.execute("SELECT ProjectID FROM ProjectFund WHERE ProjectID IN (SELECT ProjectID FROM Projects WHERE ProjectStatus = 'Approved' AND CreatorUserID = %s) AND FundSendDone > 0", (current_user_id,))
...
def check_a_project_fund_applied_or_not(project_id):
    ...
    cursor.execute(
        "SELECT ProjectFundID FROM ProjectFund WHERE RequestForFundDone > 0 AND ProjectID = %s", (project_id,))
...
def create_fund_request_for_specific_project():
    ...
    review_data = (data['ProjectID'], data['TotalBudget'], data['HonorariumOfPI'], data['HonorariumOfCoPI'], data['PiSignatureDate'],
                   data['ChairmanOfDepartmentSignatureDate'], data['TotalHonorarium'], data['RequestForFundDone'], data['FundRecievedDone'], 0)
...
def get_all_my_funded_projects():
    ...
    cursor.execute("SELECT ProjectID , CodeByRTC , ProjectTitle FROM Projects WHERE ProjectID IN (SELECT ProjectID FROM ProjectFund WHERE ProjectID IN (SELECT ProjectID FROM Projects WHERE ProjectStatus = 'Approved' AND CreatorUserID = %s) AND FundSendDone > 0)", (current_user_id,))
    funded_project = cursor.fetchall()
...
def check_a_project_fund_confirmation_send_or_not(project_id):
    ...
    cursor.execute(
        "SELECT ProjectFundID FROM ProjectFund WHERE FundRecievedDone > 0 AND ProjectID = %s", (project_id,))
...
def update_confirm_fund_recieved(project_id):
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute(
        "UPDATE ProjectFund SET FundRecievedDone = 1 WHERE ProjectID = %s", (project_id,))
    conn.commit()
    cursor.close()
    conn.close()
...
def get_admin_fund_dashboard():
    ...
    cursor.execute(
        "SELECT ProjectID FROM ProjectFund WHERE RequestForFundDone > 0")
    fund_request_queue = cursor.fetchall()
    print(fund_request_queue)
    fund_request_queue = len(fund_request_queue) - funded_projects

    cursor.execute(
        "SELECT ProjectID FROM ProjectFund WHERE FundRecievedDone > 0")
...
def get_admin_fund_queue_list():
    ...
    cursor.execute(
        "SELECT * FROM ProjectFund WHERE RequestForFundDone > 0 AND FundSendDone = 0")
...
def check_a_project_fund_send_or_not(project_id):
    ...
    cursor.execute(
        "SELECT ProjectFundID FROM ProjectFund WHERE FundSendDone > 0 AND ProjectID = %s", (project_id,))
...
def update_fund_send(project_id):
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute(
        "UPDATE ProjectFund SET FundSendDone = 1 WHERE ProjectID = %s", (project_id,))
    conn.commit()
    cursor.close()
    conn.close()
