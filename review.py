...
def get_revieweruserid_for_specific_project(project_id):
    ...
    cursor.execute(
        "SELECT ReviewerUserID FROM ProjectListWithReviewerID WHERE ProjectID = %s", (project_id,))
...
def review_dashboard():
    ...
    cursor.execute(
        "SELECT COUNT(*) AS total_projects_to_review FROM ProjectListWithReviewerID WHERE ReviewerUserID = %s", (current_user_id,))
    total_projects_to_review = cursor.fetchone()
    print(total_projects_to_review['total_projects_to_review'])

    cursor.execute(
        "SELECT COUNT(*) AS completed_reviews FROM Review WHERE ReviewerUserID = %s AND Comments IS NOT NULL", (current_user_id,))
    completed_reviews = cursor.fetchone()
    print(completed_reviews['completed_reviews'])

    pending_reviews = {
        'pending_reviews': total_projects_to_review['total_projects_to_review'] - completed_reviews['completed_reviews']}
    ...
    if len(user_projects) == 0:
        ...
        projects_in_review = [project['ProjectID']
        ...
        review_queue = {'review_queue': (
            review_queue['review_queue'] - review_done['review_done'])}
...
def update_picanviewornot(project_id):
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute(
        "UPDATE Review SET PiCanViewOrNot = 1 WHERE ProjectID = %s", (project_id,))
    conn.commit()
    cursor.close()
    conn.close()
...
def get_all_projects_have_to_review():
    ...
    current_user_id = get_jwt_identity()
    cursor.execute(
        "SELECT * FROM ProjectListWithReviewerID WHERE ReviewerUserID = %s", (current_user_id,))
    ProjectHaveToReviewList = cursor.fetchall()
    # retrieve all projects that have to be reviewed with ProjectHaveToReviewList
    projects_in_review = [project['ProjectID']
                          for project in ProjectHaveToReviewList]
    print(projects_in_review)
...
def check_a_project_reviewed_or_not(project_id, user_id):
    ...
    cursor.execute(
        "SELECT * FROM Review WHERE ProjectID = %s AND ReviewerUserID = %s", (project_id, user_id))
...
def get_all_reviews_for_specific_reviewer():
    ...
    conn = get_db()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT * FROM Review WHERE ProjectID = %s AND ReviewerUserID = %s",
                   (data['ProjectID'], data['ReviewerUserID']))
    reviews = cursor.fetchall()
    cursor.close()
    conn.close()
    return jsonify({'reviews': reviews, 'statuscode': 200}), 200
...
def review_panel_overview():
    ...
    cursor.execute(
        "SELECT COUNT(DISTINCT ProjectID) AS assigned_reviewer FROM ProjectListWithReviewerID")
    ...
    remaining_projects = totalNumOfProjects['totalNumOfProjects'] - \
        assigned_reviewer[0]['assigned_reviewer']
...
def get_all_projects_have_to_set_reviewer():
    ...
    cursor.execute(
        "SELECT * FROM Projects WHERE ProjectID NOT IN (SELECT DISTINCT ProjectID FROM ProjectListWithReviewerID)")
...
def get_all_projects_reviewer_given_review():
    ...
    cursor.execute(
        "SELECT * FROM Projects WHERE ProjectID IN( SELECT ProjectID FROM Review GROUP BY ProjectID HAVING COUNT(*) = 3 )")
...
def get_review_project_gantt(project_id):
    ...
    cursor.execute("SELECT ProjectID FROM ProjectListWithReviewerID WHERE ProjectID = %s AND ReviewerUserID = %s",
